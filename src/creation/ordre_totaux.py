...
def creation_vote_ordre_totaux(n, m, polarisation):
    if n % 2 == 1 or m % 2 == 1:
        print("n et m doivent être pairs !")
        return

    # On choisit deux camps dominants aléatoirement
    camp_1 = np.random.permutation(np.array([i for i in range(m)]))
    camp_2 = camp_1[::-1]
    ...
    if polarisation == 0:
        ...
        for i in range(n):
            ...
            camp = np.random.choice([0, 1], p=[1 - polarisation / 2,
                                               polarisation / 2])  # On divise par deux pour avoir une polarisation totale seulement en 1 et pas en 0.5
            if camp == 0:
                vote[i] = camp_1
            else:
                vote[i] = camp_2
